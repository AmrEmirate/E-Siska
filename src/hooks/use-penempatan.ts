...
export function usePenempatan() {
  ...
  const fetchPenempatan = useCallback(
    async (kelasId?: string, tahunAjaranId?: string) => {
      setLoading(true);
      try {
        ...
        toast({
          ...
          description:
            "Terjadi kesalahan saat mengambil data penempatan siswa.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const updatePenempatan = async (
    id: string,
    penempatanData: Partial<Penempatan>
  ) => {
    try {
