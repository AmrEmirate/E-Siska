...
export default function RaporDetailPage() {
  ...
  const fetchRapor = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch rapor", error)
      toast({
        variant: "destructive",
  ...
  const handleFinalize = async () => {
    if (!activeTahunAjaran || !user?.id) return
    if (!confirm("Apakah Anda yakin ingin memfinalisasi rapor ini? Data tidak dapat diubah setelah finalisasi.")) return
    ...
    try {
      ...
      toast({
        ...
        description: "Pastikan semua data wajib (catatan & kokurikuler) sudah diisi.",
      })
    } finally {
      setProcessing(false)
    }
  ...
  const handleDefinalize = async () => {
    if (!activeTahunAjaran || !user?.id) return
    if (!confirm("Batalkan status finalisasi? Rapor akan kembali menjadi Draft.")) return
    ...
    try {
      ...
      console.error("Failed to definalize", error)
      toast({
        variant: "destructive",
    ...
    <div className="min-h-screen bg-gray-100 pb-20 print:bg-white print:pb-0">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-20 print:hidden">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/dashboard/homeroom/students" className="p-2 hover:bg-gray-100 rounded-full transition-colors">
              <ArrowLeft size={20} className="text-gray-600" />
            </Link>
            <div>
              <h1 className="text-lg font-bold text-gray-900">{rapor.infoSiswa.nama}</h1>
              <p className="text-xs text-gray-500">{rapor.infoSiswa.nis} • {rapor.infoSiswa.kelas}</p>
            </div>
          </div>
      ...
      <div className="max-w-[210mm] mx-auto bg-white mt-8 shadow-lg print:shadow-none print:mt-0 p-[20mm] min-h-[297mm]">
        ...
        <div className="text-center border-b-2 border-gray-800 pb-6 mb-8">
          <h2 className="text-2xl font-bold uppercase tracking-wide mb-1">Laporan Hasil Belajar Siswa</h2>
          <h3 className="text-xl font-semibold text-gray-700">SMA Negeri Contoh Indonesia</h3>
          <p className="text-sm text-gray-500 mt-2">Jl. Pendidikan No. 1, Jakarta Selatan. Telp: (021) 1234567</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-12 gap-y-2 mb-8 text-sm">
          <div className="flex">
            ...
            <span className="font-semibold uppercase">{rapor.infoSiswa.nama}</span>
          </div>
          <div className="flex">
            <span className="w-32 font-medium text-gray-600">Kelas / Semester</span>
            <span className="mr-2">:</span>
        ...
        {/* A. Nilai Akademik */}
        <div className="mb-8">
          <h4 className="font-bold text-lg mb-3 border-l-4 border-blue-600 pl-3">A. Nilai Akademik</h4>
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="border border-gray-300 px-4 py-2 w-16 text-center">KKM</th>
                <th className="border border-gray-300 px-4 py-2 w-16 text-center">Nilai</th>
                <th className="border border-gray-300 px-4 py-2 w-16 text-center">Predikat</th>
                <th className="border border-gray-300 px-4 py-2 text-left">Deskripsi Capaian Kompetensi</th>
              </tr>
            </thead>
            <tbody>
              {rapor.nilaiAkademik.map((nilai, idx) => (
                <tr key={idx}>
                  ...
                  <td className="border border-gray-300 px-4 py-2 font-medium">{nilai.mapel}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center text-gray-500">{nilai.kkm}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center font-bold">{nilai.nilaiAkhir}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center">{nilai.predikat}</td>
        ...
        {/* B. Ekstrakurikuler */}
        <div className="mb-8">
          <h4 className="font-bold text-lg mb-3 border-l-4 border-blue-600 pl-3">B. Ekstrakurikuler</h4>
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-4 py-2 w-12 text-center">No</th>
                <th className="border border-gray-300 px-4 py-2 text-left">Kegiatan Ekstrakurikuler</th>
                <th className="border border-gray-300 px-4 py-2 w-24 text-center">Predikat</th>
            ...
            <tbody>
              {rapor.ekstrakurikuler.length > 0 ? (
                rapor.ekstrakurikuler.map((ekskul, idx) => (
                  <tr key={idx}>
                    <td className="border border-gray-300 px-4 py-2 text-center">{idx + 1}</td>
                    <td className="border border-gray-300 px-4 py-2 font-medium">{ekskul.nama}</td>
                    <td className="border border-gray-300 px-4 py-2 text-center">{ekskul.nilai}</td>
                    <td className="border border-gray-300 px-4 py-2 text-xs text-gray-600">{ekskul.deskripsi}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={4} className="border border-gray-300 px-4 py-4 text-center text-gray-500 italic">Tidak mengikuti ekstrakurikuler</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="mb-8 w-1/2">
          <h4 className="font-bold text-lg mb-3 border-l-4 border-blue-600 pl-3">C. Ketidakhadiran</h4>
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <tbody>
              <tr>
                <td className="border border-gray-300 px-4 py-2 w-40">Sakit</td>
                <td className="border border-gray-300 px-4 py-2 text-center font-medium">{rapor.ketidakhadiran.sakit} hari</td>
              </tr>
              <tr>
                <td className="border border-gray-300 px-4 py-2">Izin</td>
                <td className="border border-gray-300 px-4 py-2 text-center font-medium">{rapor.ketidakhadiran.izin} hari</td>
              </tr>
              <tr>
                <td className="border border-gray-300 px-4 py-2">Tanpa Keterangan</td>
                <td className="border border-gray-300 px-4 py-2 text-center font-medium">{rapor.ketidakhadiran.tanpaKeterangan} hari</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="mb-8 break-inside-avoid">
          <h4 className="font-bold text-lg mb-3 border-l-4 border-blue-600 pl-3">D. Catatan Wali Kelas</h4>
          {isFinal ? (
          ...
          {/* Print-only version of textarea content if not final (though usually final is required for print) */}
          {!isFinal && (
        ...
        <div className="mb-12 break-inside-avoid">
          <h4 className="font-bold text-lg mb-3 border-l-4 border-blue-600 pl-3">E. Projek Penguatan Profil Pelajar Pancasila</h4>
          {isFinal ? (
             <div className="border border-gray-300 rounded p-4 bg-gray-50 text-sm min-h-[80px]">
        ...
        <div className="flex justify-between text-center mt-16 break-inside-avoid">
          ...
          <div>
            <p className="mb-20">Jakarta, {new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}<br/>Wali Kelas</p>
