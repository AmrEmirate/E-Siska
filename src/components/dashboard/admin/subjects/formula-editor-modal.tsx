...
export function FormulaEditorModal({ isOpen, onClose, mapelId, mapelName }: FormulaEditorModalProps) {
  ...
  const fetchSkema = async () => {
    ...
    try {
      ...
      // Auto increment order based on last component
      if (response.data.data.Komponen.length > 0) {
        const lastOrder = Math.max(...response.data.data.Komponen.map((c: Component) => c.urutan))
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {skema.Komponen.length === 0 ? (
                  <p className="text-gray-400 italic text-sm">Belum ada komponen penilaian.</p>
                ) : (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 text-xs mt-1">
                              <span className={`px-2 py-0.5 rounded-full ${comp.tipe === 'INPUT' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'}`}>
                                {comp.tipe}
                              </span>
                              {comp.formula && (
              ...
              <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
                ...
                <form onSubmit={handleAdd} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Nama Komponen</label>
                      <input
                        type="text"
                        value={newName}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Tipe Nilai</label>
                    <div className="flex gap-4">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="text-blue-600"
                        />
                        <span className="text-sm">Hasil Kalkulasi (Otomatis dari rumus)</span>
                      </label>
                    </div>
                  </div>
                  ...
                  {newType === "READ_ONLY" && (
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                      ...
                      <p className="text-xs text-blue-600 mt-2">
                        Gunakan nama komponen lain sebagai variabel. Contoh: <code>mean(t1, t2)</code> atau <code>(uts + uas) / 2</code>
                      </p>
                    </div>
                  )}
