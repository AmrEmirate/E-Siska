...
export default function TeacherAnnouncementsPage() {
  ...
  const filteredAnnouncements = announcements.filter((ann) => {
    if (filter === "ALL") return ann.target === "GURU" || ann.target === "SEMUA"
    ...
    <div className="p-8">
      ...
      <div className="mb-6">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition ${
              filter === "GURU" ? "bg-red-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
            }`}
          >
            Untuk Guru
          </button>
      ...
      <div className="space-y-4">
        {filteredAnnouncements.length === 0 ? (
          <div className="card p-12 text-center text-gray-500">Belum ada pengumuman.</div>
        ) : (
            ...
            <div key={announcement.id} className="card p-6 hover:shadow-md transition">
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-2">{announcement.isi}</p>
                </div>
              </div>
              <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200">
                <p className="text-xs text-gray-500">
                  Target: {announcement.target} • {announcement.createdAt ? new Date(announcement.createdAt).toLocaleDateString("id-ID") : "-"}
                </p>
              </div>
            </div>
          ))
