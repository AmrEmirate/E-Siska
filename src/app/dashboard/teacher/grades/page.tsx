...
interface StudentGrade {
  ...
  nis: string
  nama: string
  grades: Record<string, number> // komponenId -> nilai
}
...
export default function GradesPage() {
  ...
  useEffect(() => {
    const fetchInitData = async () => {
      try {
        // In real app, fetch classes assigned to this teacher
        // For demo, we fetch all classes or mock
        // const classesRes = await apiClient.get("/kelas") 
        setClasses([{ id: "cls-1", name: "10 IPA 1" }, { id: "cls-2", name: "10 IPA 2" }]) // Mock for now
        ...
        toast({
          ...
          description: "Terjadi kesalahan saat mengambil data kelas atau mata pelajaran.",
        })
      }
    }
    fetchInitData()
  ...
  // 2. Fetch Skema & Grades when Class & Subject selected
  useEffect(() => {
  ...
  const fetchGradesAndSkema = async () => {
    ...
    try {
      ...
      // Fetch Grades
      const gradesRes = await apiClient.get(`/nilai/kelas/${selectedClass}/mapel/${selectedSubject}`)
      setGradeData(gradesRes.data.data)
    } catch (error) {
      console.error("Failed to fetch grades/skema", error)
      setSkema(null)
      setGradeData([])
      toast({
        variant: "destructive",
  ...
  const handleGradeChange = (siswaId: string, komponenId: string, value: string) => {
    ...
    if (isNaN(numValue) || numValue < 0 || numValue > 100) {
      // Optionally show a warning or just don't update
      return
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fetchGradesAndSkema() // Refresh to get calculated values
    } catch (error) {
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Input Nilai Siswa</h1>
        <p className="text-gray-600">Pilih kelas dan mata pelajaran untuk mulai mengisi nilai.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700">Pilih Mata Pelajaran</label>
            <select
      ...
      {selectedClass && selectedSubject && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50/50">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Lembar Penilaian</h2>
              <p className="text-sm text-gray-500 mt-1">Pastikan semua nilai terisi dengan benar sebelum menyimpan.</p>
            </div>
            <button 
              onClick={handleSave}
            ...
            {loading ? (
              ...
              <div className="p-8">
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Skema penilaian belum dibuat untuk mata pelajaran ini. Silakan hubungi Admin untuk membuat skema penilaian terlebih dahulu.
                  </AlertDescription>
                </Alert>
              </div>
            ) : (
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50 text-gray-700 border-b border-gray-200 font-semibold uppercase tracking-wider">
                  <tr>
                    <th className="px-6 py-4 sticky left-0 bg-gray-50 z-10 w-24">NIS</th>
                    <th className="px-6 py-4 sticky left-[96px] bg-gray-50 z-10 min-w-[200px]">Nama Siswa</th>
                    {skema.Komponen.sort((a, b) => a.urutan - b.urutan).map(comp => (
                      <th key={comp.id} className="px-6 py-4 text-center min-w-[140px]">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {gradeData.length === 0 ? (
                    <tr>
                      <td colSpan={2 + skema.Komponen.length} className="px-6 py-12 text-center text-gray-500">
                        <div className="flex flex-col items-center justify-center">
                          <div className="bg-gray-100 p-4 rounded-full mb-3">
                            <AlertCircle className="h-8 w-8 text-gray-400" />
                          </div>
                          <p className="text-lg font-medium text-gray-900">Belum ada siswa</p>
                          <p className="text-sm text-gray-500">Tidak ada siswa yang terdaftar di kelas ini.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    gradeData.map((student) => (
                      <tr key={student.siswaId} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 font-medium text-gray-900 sticky left-0 bg-white">{student.nis}</td>
                        <td className="px-6 py-4 text-gray-700 sticky left-[96px] bg-white">{student.nama}</td>
                        {skema.Komponen.sort((a, b) => a.urutan - b.urutan).map(comp => (
                          <td key={comp.id} className={`px-4 py-3 text-center ${comp.tipe === 'READ_ONLY' ? 'bg-gray-50/50' : ''}`}>
                            {comp.tipe === 'INPUT' ? (
                              <div className="relative flex justify-center group">
                                <input
                                  type="number"
                                  value={student.grades[comp.id] ?? ''}
                                  onChange={(e) => handleGradeChange(student.siswaId, comp.id, e.target.value)}
                              ...
                              <span className="font-bold text-gray-700">
                                {student.grades[comp.id] !== undefined ? student.grades[comp.id].toFixed(2) : '-'}
                              </span>
                            )}
                          </td>
                        ))}
